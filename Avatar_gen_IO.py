...
# Убираем лишние варианты (они читались из файла avatar_gen.IO, потому
# просто уберу).
...
def generate_avatar():
    ...
    # 3. Прическа
    hair_style = show_menu(
        params["hair_style"] +
        params["alt_hair_style"],
        "Стиль прически")
    options["hair"] = PARAM_MAPPING["hair_style"][gender].get(
        hair_style, "short01")
    ...
    if show_menu(["true", "false"], "Добавить очки?") == "true":
        ...
        options["accessoriesType"] = PARAM_MAPPING["glasses"].get(
            glasses_type, "variant05")
        # Цвет очков как у волос
        options["accessoriesColor"] = options["hairColor"]
